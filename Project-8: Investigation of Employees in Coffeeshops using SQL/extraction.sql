-- S1: Extract employees with a salary of > 20,000 USD.
SELECT * FROM employees WHERE salary > 20000;
-- S2: Extract only the employees working in Common Grounds Coffeeshop.
SELECT * FROM employees WHERE coffeeshop_id = 1;
-- S3: Extract only the employees working in Common Grounds Coffeeshop and making a salary of  > 30,000 USD.
SELECT * FROM employees WHERE coffeeshop_id = 1 and salary > 30000;
-- S4: Extract only the male employees working in Common Grounds CoffeeShop, making a salary of > 40,000 USD.
SELECT * FROM employees WHERE coffeeshop_id = 1 AND salary > 40000 AND gender = 'M';
-- S5: Extract all the employees who work in Common Grounds or make more than 50,000 USD or are male.
...
-- S11: Extract all employees who make between 25,000 and 55,000 USD.
SELECT * FROM employees WHERE salary BETWEEN 25000 AND 55000;
...
-- S13: Find the top 5 highest paid employees.
...
-- S17:	Extract the full name of the employees. If a person makes < 20,000 USD, then true; otherwise, false.
SELECT first_name || ' '  || last_name as full_name, salary < 20000 as less_than_20k
FROM employees ;
-- S18:	Extract the full name of the employees. If a person is a female and makes less than 60,000 USD, then true.
SELECT first_name || ' ' || last_name as full_name,
(gender = 'F' AND salary < 60000) as less_than_50k_female FROM employees;
...
-- S21: Return the email addresses beginning from the 2nd character.
SELECT email, SUBSTRING(email FROM 2) FROM employees;
...
-- S24: Fill the null emails as MISSING EMAIL INFO.
SELECT email, COALESCE(email, 'MISSING EMAIL INFO') FROM employees;
...
-- S34: Find the number of employees and the average, minimum, maximum, and total salaries of coffeeshops with more than 100 employees, where coffeeshops are sorted in descending order by number of employees.
...
HAVING COUNT(employee_id) > 100 ORDER BY COUNT(employee_id) DESC;
-- S35: Extract only the coffeeshops with a minimum salary of less than 5,000 USD.
SELECT coffeeshop_id, COUNT(employee_id) as nuOfEmp, AVG(salary) as avg_salary, MIN(salary) as min_salary,
MAX(salary) as max_salary, SUM(salary) as sum_salary FROM employees GROUP BY coffeeshop_id
HAVING MIN(salary) < 5000 ORDER BY COUNT(employee_id) DESC;
-- S36: Return employee_id, full_name, salary ,and pay_category, i.e high pay or low pay If pay is less than 10,000 USD, then low pay, otherwise high pay. Sort the return by salaries in descending order.
...
-- S37: If pay is less than 10k, then low pay
-- if between 10k-40k inclusive, then medium pay
-- if over 40k, then high pay
SELECT employee_id, first_name || ' ' || last_name as full_name, salary,
CASE
WHEN salary < 10000 THEN 'low pay'
WHEN salary > 10000 AND salary < 40000 THEN 'medium pay'
ELSE 'high pay'
END as pay_category
FROM employees ORDER BY salary DESC;
...
SELECT
	SUM(CASE WHEN salary < 10000 THEN 1 ELSE 0 END) AS low_pay,
	SUM(CASE WHEN salary BETWEEN 10000 AND 40000 THEN 1 ELSE 0 END) AS medium_pay,
	SUM(CASE WHEN salary > 40000 THEN 1 ELSE 0 END) AS high_pay
FROM employees;
...
-- S54: Return all employees who make over 15,000 USD and work in US coffeeshops.
SELECT * from employees WHERE salary > 15000 AND coffeeshop_id IN
